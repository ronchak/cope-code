...
const TOOL_DESCRIPTIONS: Readonly<Record<ToolName, string>> = {
  ...
  lsp_query: "Run one bounded, read-only language-server hover, definition, reference, or symbol query.",
...
export function renderBootstrapContract(options: BootstrapContractOptions): string {
  ...
  return [
    ...
    "```",
    "",
    "You may batch only independent read-only operations (list_files, search_text, read_file, git_status, git_diff, lsp_query). Request apply_patch, run_command, request_user_input, request_capability, and complete_task alone so you can observe each material result before deciding the next action. Never retry an operation_id.",
    "",
