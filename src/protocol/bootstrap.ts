...
const TOOL_DESCRIPTIONS: Readonly<Record<ToolName, string>> = {
  ...
  edit_text: "Atomically replace exact text in one hash-guarded approved text file with an occurrence-count guard.",
...
export function renderBootstrapContract(options: BootstrapContractOptions): string {
  ...
  return [
    ...
    "```",
    "",
    "You may batch only independent read-only operations (list_files, search_text, read_file, git_status, git_diff). Request edit_text, apply_patch, run_command, request_user_input, request_capability, and complete_task alone so you can observe each material result before deciding the next action. Prefer edit_text for small exact replacements; use apply_patch for creates, deletes, or whole-file/multi-file transactions. Never retry an operation_id.",
    "",
