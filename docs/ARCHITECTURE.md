...
8. In `edit`/`auto`, the model must submit a structured plan before its first `apply_patch` or `run_command`. The runtime presents the exact summary, ordered steps, anticipated mutations, and validation plan to the user, then durably records the plan hash and decision. Rejection grants no authority; a revised plan requires a new submission and decision. Read-only discovery remains available before approval.
9. After plan approval, the runtime still evaluates every requested operation against organization policy, repository policy, and the session grant. Plan approval is a sequencing boundary, not a policy bypass or capability grant.
10. The tool host revalidates local facts, executes the operation, records actual results, and places source-bearing output through the disclosure guard. For `run_command`, explicitly granted `sideEffects: true` validation may run in `edit`/`auto` and may create ordinary Git-ignored artifacts. Every command is bracketed by nested-Git plus Git-visible/nonignored, keyed policy-hidden protected, and Git-control integrity checks; `sideEffects: false` additionally binds a bounded ordinary-ignored-file inventory. Disallowed or unverifiable drift becomes recovery-required and cannot be trusted as validation. Intentional source-mutating commands remain outside v1 until a versioned write-scope/checkpoint contract exists.
11. Results return through the same transport. After plan approval, the loop continues without per-step approval while every operation remains in the existing grant and budget.
12. `complete_task` triggers local verification. A failed check becomes a structured result for Copilot to act on; only a successful local check makes the session `completed`.
