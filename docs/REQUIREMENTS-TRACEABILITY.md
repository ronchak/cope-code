...
| FR-007 | session/artifact/journal/runtime recovery; agent-runtime tests; seeded reliability harness | Automated for modeled crash states with hard exits at selected high-risk durable runtime, journal, and patch/checkpoint boundaries; exhaustive power-loss/process-kill injection at every commit point remains. |
...
| Reliability | explicit state machine, intent/journal/artifact ordering, hard-exit boundary tests, seeded fuzz/soak, and selected-source coverage floors | exhaustive power-loss/process-kill injection at every commit point and live browser recovery |
